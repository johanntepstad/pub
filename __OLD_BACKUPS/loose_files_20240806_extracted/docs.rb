...
required_gems = ["kramdown", "async", "rubyzip"]
required_gems.each do |gem|
  system("gem install --user-install #{gem}") unless Gem::Specification.find_all_by_name(gem).any?
end
...
def scrape_web_page(url, site_name, output_dir, logger, depth, max_depth, visited_urls, domain, semaphore)
  ...
  semaphore.async do
    ...
    begin
      ...
      markdown_content = Kramdown::Document.new(content).to_kramdown
      ...
      links.each do |link|
        begin
          ...
          if link_domain && link_domain.include?(domain)
            # Concurrently scrape the linked pages with delay
            Async do
              sleep(REQUEST_DELAY)
              scrape_web_page(link, site_name, output_dir, logger, depth + 1, max_depth, visited_urls, domain, semaphore)
            end
          end
...
Async do
  ...
  web_urls.each do |name, details|
    Async do
      ...
      scrape_web_page(details[:url], name, OUTPUT_DIR, logger, 1, details[:depth], [], domain, semaphore)
    end
    sleep(REQUEST_DELAY) # Initial delay to avoid hammering
  end
end.wait
